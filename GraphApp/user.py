...
class UI:
    ...
    """
        ASSIGNMENT 4: MINUMAL SPANNING TREE 
        KRUSKAL ALGORITHM
    """
    ...
    #     edges2=[]
    #     l=[]
    #     for v in self._graph.parse_vertices():
    #         forest.append(v)
    #         rank.append(0)
    #     edges=dict(sorted(self._graph.get_costs.items(), key=lambda item: item[1]))
    #     print(edges)
    #     for e in edges.keys():
    #         l=[]
    #         l.append(e[0])
    ...
    #     i=0
    #     e=0
    #     print('nb vertices',self._graph.number_of_vertices())
    #     while e < self._graph.number_of_vertices()+1:
    #         if edges2[i]==edges2[-1]:
    #             break
    #         v1,v2,c=edges2[i]
    #         print(edges2[i])
    #         i = i + 1
    #         x=self.search(forest,v1)
    #         y=self.search(forest, v2)
    #         if x!=y:
    #             e+=1
    #             result.append([v1,v2,c])
    #             self.apply_union(forest,rank,x,y)
    #     for v1,v2,c in result:
    #         print("Edge:", v1, v2, end=" ")
    #         print("-", c)
    #
    #
    #
    ...
    def make_set(self,v, parent, rank):
        """
        Initializes each vertex as its own parent
        """
        parent[v] = v
        rank[v] = 0
    ...
    def find(self,v, parent):
        """
        It finds the root (representative) of the set to which vertex v belongs.
        Updating the parent of each visited vertex directly to the root.
        """
        if v != parent[v]:
            parent[v] = self.find(parent[v], parent)
        return parent[v]

    def union(self,root1, root2, parent, rank):
        """
        ensures that the smaller tree is attached to the root of the larger tree
        if the ranks of the two roots are equal, the rank of the new root is incremented.
        """
    ...
    def kruskal(self):
        """
         It first sorts the edges of the graph in ascending order based on their weights.
         Then iterates over the sorted edges and checks if adding each edge to the resulting tree would create a cycle.
         If not, it adds the edge to the result and performs the union operation
        """
        result=[]
        total=0
        edges = dict(sorted(self._graph.get_costs.items(), key=lambda item: item[1]))
        ...
        for edge in edges2:
            u, v = edge[0],edge[1]
            if u not in parent:
                self.make_set(u, parent, rank)
            if v not in parent:
                self.make_set(v, parent, rank)

        for edge in edges2:

            u, v, weight = edge[0], edge[1], edge[2]
            root1 = self.find(u, parent)
            root2 = self.find(v, parent)

            # Add the edge to the result if it does not create a cycle
            if root1 != root2:
                total+=edge[2]
                result.append(edge)
                self.union(root1, root2, parent, rank)
        print("Total cost: ", total)
        return result

    # Example usage:
    # Create a graph represented as a list of lists [origin, destination, weight]
    ...
    def breadth_first(self,source, visited):
        ...
        :param visited: dictionary for keeping track of visited vertices, keys: vertices, values: boolean values
        :return: graph object representing a connected component
        """
